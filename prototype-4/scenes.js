...
const SC1 = {
  ...
  async play(ctx, r) {
    ...
    for (let i = 0; i < C.s1.lines.length; i++) {
      ...
      await ctx.wait(L.v === 'DENIED' ? 650 : 260);
    }
    await ctx.wait(380);
    show(r.load);
    show(r.lbl);
    await ctx.wait(300);
    await runBar(ctx, r.bar, C.s1.loadTo, 2600);
    await ctx.wait(1300);
    E.typingSound = false;
  }
};
...
const SC2 = {
  ...
  async play(ctx, r) {
    ...
    for (let i = 0; i < C.s2.lines.length; i++) {
      await type(ctx, r.lines[i], '> ' + C.s2.lines[i], 50);
      await ctx.wait(220);
    }
    await ctx.wait(480);
    await type(ctx, r.observer, C.s2.observer, 20);
    await ctx.wait(380);
    for (let i = 0; i < C.s2.lines2.length; i++) {
      await type(ctx, r.lines2[i], '> ' + C.s2.lines2[i], 50);
      await ctx.wait(200);
    }
    await type(ctx, r.mission, C.s2.mission, 24);
    r.mission.classList.add('flash');
    await ctx.wait(1000);
    for (let i = 0; i < C.s2.lines3.length; i++) {
      await type(ctx, r.lines3[i], '> ' + C.s2.lines3[i], 46);
      await ctx.wait(220);
    }
    show(r.warn);
    await type(ctx, r.prompt, '> ' + C.s2.prompt, 36);
...
const SC3 = {
  ...
  async play(ctx, r) {
    ...
    for (let i = 0; i < C.s3.rows.length; i++) {
      ...
      await type(ctx, r.rowNodes[i].k, k + ': ', 74);
      await type(ctx, r.rowNodes[i].v, v, 46);
      await ctx.wait(240);
    }
    await ctx.wait(400);
    for (let i = 0; i < C.s3.sysLines.length; i++) {
      await type(ctx, r.sysNodes[i], '> ' + C.s3.sysLines[i], 74);
      await ctx.wait(260);
    }
    await ctx.wait(300);
    E.typingSound = false;
